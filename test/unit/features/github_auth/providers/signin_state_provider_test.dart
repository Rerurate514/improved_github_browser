...
void main() {
  ...
  group("signInStateNotifier", () {
    ...
    test("トークンの取得中にエラーが発生した場合にAuthResultにエラー情報が含まれる", () async {
      when(
        mockGithubSecureRepository.getToken(),
      ).thenThrow(Exception("Test error during token fetch"));
    ...
    test("サインインに成功した場合にAuthResult.successが返される", () async {
      ...
      when(
        mockGithubAuthRepository.signIn(),
      ).thenAnswer((_) => Future.value(AuthResult.success("test_token")));
    ...
    test("サインインに失敗した場合にAuthResult.failureが返される", () async {
      ...
      when(
        mockGithubAuthRepository.signIn(),
      ).thenAnswer((_) => Future.value(AuthResult.failure("Failed to get token")));
